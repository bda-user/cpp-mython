...
private:
    std::string name_{""s};
    std::unique_ptr<Statement> value_ = nullptr;
};
...
public:
    ...
    void CompoundImpl(Arg&& arg, Args&&... args) {
        AddStatement(std::forward<Arg>(arg));
//        if constexpr (sizeof...(args) != 0) {
//            CompoundImpl(std::forward<Args>(args)...);
//        }
        (..., CompoundImpl(std::forward<Args>(args)));
    }
